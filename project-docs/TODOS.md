...
- [~] step5 — 스킬(다중+우선순위+마나/쿨). 직업당 하나씩 검증. **설계 진행중** → 확정 결정·미결 6개 = `docs/superpowers/specs/step5-decisions-captured.md`(로컬, gitignore). 스코프=엔진+스킬(우선순위=데이터순서), 평타가 마나충전, 직업별 스킬(사제 도트힐/전사 방깎+딜/법사 딜/가디언 도발+방버프) → 버프·디버프 지속 시스템 인스코프.
- [ ] step6 — 상호작용 동사(마법사 방깎부터).
- [ ] step7 — 스킬 레벨업. step8 — 2차 전직 트리. step9 — 속성 degree화. step10 — (선택) 유물.

## 베이스 후속
- [ ] **밸런스 튜닝** — `curve`/`monsters`(계수)/`stages`(슬롯) placeholder 수치 확정. 현재 시뮬상 3유닛 파티가 S4+ 전멸, 4유닛 균형도 S5 7/30 → 후반 스테이지 너무 가파름. 트레잇별 RPS 갈림(§8 검증기준 3)도 측정.
