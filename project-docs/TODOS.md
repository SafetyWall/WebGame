...
## ★ 다음 작업: UI/UX 개편 (2026-06-03 사용자 지시)
현 UI = 순수 HTML 문자열 렌더(`src/ui/game.js`) + 이벤트 위임(`src/ui/main.js`). 평문 `<li>` 나열 = 인지성↓, 스킬/특성 정보 안 보임.
- [ ] **캐릭터 카드(박스)** — 평문 리스트 → 카드 레이아웃. 시각 인지성.
- [ ] **스킬 정보 노출** — 현재 스킬 옵션·수치(power/cost/cd/effect) 못 봄 → 표시.
- [ ] **적 특성 툴팁** — prep "다가올 적 [특성]"에 hover 시 능력 설명 박스(현재 이름만, 무슨 능력인지 모름).
- [ ] **드래그 순서변경** — 현 ▲▼ 버튼 별로 → 드래그앤드롭(출전 순서 `reorderParty` + 스킬 우선순위 `reorderSkill`).
- [ ] **캐릭터 상세 팝업(모달)** — 캐릭터 클릭 → 능력치 상세 + 스킬레벨업 버튼. **레벨업 시 변화 능력치 미리보기(before→after)** 같이 표시.
- [ ] **기본패널 hover 툴팁** — 스킬/특성에 마우스 올리면 옆 박스로 능력 설명(모달 말고 인라인 툴팁).

**기술 메모:** 드래그·모달·툴팁 = 인터랙션 상태(hover/drag/선택 캐릭) 추가 필요 → 현 "정적 전체 재렌더" 패턴과 호환 설계 고려(상태를 main.js에 두고 부분 갱신 or 재렌더 유지+CSS). 디자인은 `project-docs/design/art.md` 참조·갱신. UI 작업이라 brainstorm(요구 명확) 스킵 가능, 단 인터랙션 패턴(재렌더 vs 부분DOM)은 첫 결정 필요.
