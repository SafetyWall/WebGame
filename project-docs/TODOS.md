...
- [~] **클래스 정체성 리워크.** ✅ **phase1(2026-06-08)**: 가디언 베이스 제거, 전사 강화제거+방벽흡수, 시그니처 독점, amp 약화, 강제직업0. ✅ **phase2 역할 확정(2026-06-08)**: **가디언 클래스 완전 삭제**(→전사 2차전직 보류), 사제=강평타+미미버프(15%/10%)+힐(평타≈전사), 마법사=유리대포 버스트(약평타·파이어볼 폭딜·관통 안티아머), 도적=순수 속도+연타(spd130·더블어택extraHit·난무·마나절단; 출혈/방어무시 제거), 궁수=디버퍼(속박 감속+주는뎀↓·독화살). 원시 DPS **법사>도적>궁수**, 사제 평타≈전사. 신규 메커니즘 extraHit·manaDrain. **잔여:**
  - **타겟팅 리워크** — 저체력추적 blunt → 평타/스킬 분리(평타만 최저HP or 스킬만) / 몹스킬 맨뒤부터 등. (몬스터 스킬과 엮임.)
  - **2차전직 시스템** — 전사→가디언(하드탱), 마법사→극단 글래스캐논 등. 가디언 스킬은 **삭제됨**(intercept/taunt 엔진 메커니즘 + warrior_might/mage_frost/priest_hot 미배선 def만 잔존 → 2차전직 때 재정의). (roadmap step8.)
  - **마나절단 활성** — 적 마나차감(`manaDrain`)은 현재 **휴면**(몹 마나 미사용). 몬스터 스킬 도입 시 활성.
  - **스탯 미세조정** — HP 격차 좁히기, DPS 정밀 = econ-sim(곡선 완화와 함께).
