...
test('출혈 = dot 효과(틱당 데미지) 정의', () => {
  assert.strictEqual(SKILLS.rogue_bleed.effects[0].type, 'dot')
  assert.ok(SKILLS.rogue_bleed.effects[0].valueRatio > 0)
  assert.ok(SKILLS.rogue_bleed.effects[0].interval > 0)
})
