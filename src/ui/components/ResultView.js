...
export function renderResultView(run, ui) {
  ...
  const banner = outcome === 'clear' ? `<h2>🎉 스테이지 ${run.stage} 클리어 — 게임 클리어! (${fmtSec(ticks)})</h2>`
    : outcome === 'win' ? `<h2>스테이지 ${run.stage} 승리! 골드 +${reward} (${fmtSec(ticks)})</h2>`
    : `<h2>패배… 런 종료 (${fmtSec(ticks)})</h2>`
